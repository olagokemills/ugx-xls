...
exports.DEFAULT_CSV_OPTIONS = void 0;
exports.jsonToCsvString = jsonToCsvString;
...
function jsonToCsvString(rows, opts) {
    var _a;
    const options = Object.assign(Object.assign({}, exports.DEFAULT_CSV_OPTIONS), (opts || {}));
    if (!Array.isArray(rows))
        throw new errors_1.SpreadsheetError('rows must be an array', 'INVALID_INPUT');
    if (rows.length === 0)
    ...
    if (options.sanitizeHeaders)
        headers = headers.map(sanitizer_1.sanitizeHeader);
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        for (let c = 0; c < headers.length; c++) {
            if (c + 1 > options.maxCols)
                throw new errors_1.SpreadsheetError('Column limit exceeded during write', 'LIMIT_COLS');
            const h = headers[c];
            let v = r[h];
            ...
            v = (0, sanitizer_1.enforceLimits)(v, options.maxCellChars);
